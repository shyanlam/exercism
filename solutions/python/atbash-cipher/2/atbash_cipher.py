...
ATBASH_LOOKUP: dict[str, str] = dict(zip(ALPHA, reversed(ALPHA))) | dict(zip(DIGIT, DIGIT))

def make_atbash_table():
    return str.maketrans(ALPHA + DIGIT, ''.join(reversed(ALPHA)) + DIGIT)
