...
ROW_LETTERS: str = "ABCD"
ROW_COUNTS: int = len(ROW_LETTERS)
...
def generate_seat_letters(number: int) -> Generator[str, None, None]:
    ...
    """
    cycle = itertools.cycle(ROW_LETTERS)
...
def generate_seats(number: int) -> Generator[str, None, None]:
    ...
    """
    rows = (row for row in itertools.count(start=1) if row != 13 for _ in range(ROW_COUNTS))
