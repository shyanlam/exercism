...
def count_failed_students(student_scores: list[int]) -> int:
    ...
    return len(list(filter(lambda score: score <= FAILED_SCORE, student_scores)))
...
def letter_grades(highest: int) -> list[int]:
    ...
    return list(range(FAILED_SCORE + 1, highest, step))
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    return [f"{index + 1}. {info[0]}: {info[1]}" for index, info in enumerate(zip(student_names, student_scores))]
